...
const free_play_mode_toggle = computed({
  get: () => game_mode.value === "free_play",
  set: (value: boolean) => {
    if (value)
      game_mode.value = "free_play"
    else
      game_mode.value = "matching"
  }
})
...
<template>
  <v-card class="mx-auto my-4 px-2" max-width="600">
    ...
    <p class="text-body-2 mt-2">
      If you like, you can play the game in 'free play' mode.
      In this mode, you won't have a face to try to match, and you can experiment
      with putting the pieces together in any way you like.
    </p>
    <v-switch
        v-model="free_play_mode_toggle"
        class="d-flex justify-center"
        :inset="true"
        :color="free_play_mode_toggle? 'success' : ''"
        label="Use free play mode"
    />

    <div v-if="!free_play_mode_toggle">
      <p class="text-body-2">
    ...
    <v-expand-transition class="advanced_settings my-4">
      <v-list v-if="custom_settings">
        ...
        <v-list-item v-if="!free_play_mode_toggle">
          <v-list-item-title>
            Pieces are
        ...
        <v-list-subheader :inset="true" v-if="!free_play_mode_toggle">About Potato Thieves</v-list-subheader>
        <v-list-item v-if="!free_play_mode_toggle">
          <v-list-item-title>
        ...
        <v-list-item v-if="needy_potato_enabled && game_mode !== 'free_play'">
          <v-list-item-title>
        ...
        <v-list-subheader :inset="true" v-if="!free_play_mode_toggle">About the Difficulty</v-list-subheader>
        <v-list-item v-if="!free_play_mode_toggle">
          <v-list-item-title>
