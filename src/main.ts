...
export class Game extends Engine {
  ...
  upload_permission: boolean = false;
  ...
  initialize() {
    ...
    this.start(loader)
      .then(() => Promise.allSettled([this.activateCamera(), this.api_client.initialize(this.game_data)]))
  ...
  async activateCamera() {
    ...
    if (this.webcam.srcObject) {
      ...
      this.upload_permission = true;  // TODO: Nice interface for permission request
    }
  }
  ...
  reportToAPI(clickEvent: PointerEvent) {
    ...
    if (!this.upload_permission) {
      console.log("Not reporting data", game_data, click_data, image)
    } else {
      console.log("Reporting to API", game_data, click_data, image)
